...
export default function StudentClub({ setMessage, yearArray }) {
	...
	useEffect(() => {
		if (user) {
			const q = query(collection(db, "student-club"), where("uid", "==", user.uid));
	...
	return (
		<>
			<Layout title={'Club Details'} className="flex items-center">
				<form className='m-auto w-11/12 bg-white rounded-xl shadow-lg px-6 py-6 form-custom-height-parent h-5/6 overflow-hidden'>
					...
					<div className='grid grid-cols-2 gap-x-6 form-custom-height overflow-hidden'>
						...
						<div className='flex flex-col gap-y-6 py-1 overflow-y-scroll'>
							{
								clubArr.length == 0 ? <h1 className='mt-5 text-xl font-bold text-center'>No results found!</h1> : clubArr.map((club, index) => {
					...
					<div className={redirect ? 'grid grid-cols-2 gap-x-6' : 'grid grid-cols-1 gap-x-6'}>
						...
						{redirect && <Button loading={loading} text={"Next"} className={"w-full mt-6"} handler={() => { router.push('/student/training?redirect=true') }} />}
					</div>
					{redirect && <p className='mt-3 text-center'><b className='cursor-pointer' onClick={() => { router.push('/student/training?redirect=true') }}>Click here</b> if you are not core team member of any club</p>}
				</form>
			</Layout>
		</>
	)
