...
export default function StudentInternship({ setMessage, yearArray }) {
	...
	const handleSubmit = async () => {
		if (user) {
			setLoading(true)
			const docRef = await addDoc(collection(db, "student-internship"), { ...formState, certificate: certificateURL, uid: user.uid })
				.then(() => {
					setCertificateURL("")
	...
	const handleFileUpload = (e) => {
		if (formState.name.length != 0) {
			const storageRef = ref(storage, `internship/${formState.name.trim()}`);
			...
			setMessage("Enter name of the internship first!");
		}
	}
				...
				<form className='m-auto w-11/12 bg-white rounded-xl shadow-lg px-6 py-6 form-custom-height-parent-internship h-5/6 overflow-hidden'>
					...
					<div className='grid grid-cols-2 gap-x-6 form-custom-height-internship overflow-hidden'>
						<div className='grid grid-cols-1 gap-y-5'>
							<Input label={"Internship Name"} placeholder={"Enter name of the Internship"} id="name" value={formState.name} changeHandler={handleChange} />
							<Input label={"Company Name"} placeholder={"Enter name of the company"} id="company" value={formState.company} changeHandler={handleChange} />
							...
							<div className='grid grid-cols-6 gap-x-5'>
								<Input parentClass="col-span-5" label={"Internship Certificate"} placeholder={"Upload Certificate"} id="certificate" type='file' accept="application/pdf,application/vnd.ms-excel" changeHandler={(e) => { setFile(e.target.files[0]) }} />
