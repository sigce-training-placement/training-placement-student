...
export default function StudentAchievement({ setMessage, yearArray }) {
	...
	return (
		<>
			<Layout title={'Achievement Details'} className="flex items-center" navbar={true}>
				<form className='m-auto w-11/12 bg-white rounded-xl shadow-lg px-6 py-6 form-custom-height-parent h-5/6 overflow-hidden'>
					...
					<div className='grid grid-cols-2 gap-x-6 form-custom-height overflow-hidden'>
						...
						<div className='flex flex-col gap-y-6 py-1 overflow-y-scroll'>
							{
								achievements.length == 0 ? <h1 className='mt-5 text-xl font-bold text-center'>No results found!</h1> : achievements.map((achievement, index) => {
					...
					<div className={redirect ? 'grid grid-cols-2 gap-x-6' : 'grid grid-cols-1 gap-x-6'}>
						...
						{redirect && <Button loading={loading} text={"Next"} className={"w-full mt-6"} handler={() => { router.push('/documents?redirect=true') }} />}
					</div>
					{redirect && <p className='mt-3 text-center'><b className='cursor-pointer' onClick={() => { router.push('/documents?redirect=true') }}>Click here</b> if you haven't completed an Achievement</p>}
				</form>
			</Layout>
		</>
	)
